"""Stage 4: box walls bounding the track corridor.

Two design pieces:
- wall_params: pure NumPy. Given a Track and a side, return the pose+size of K
  box walls forming a continuous barrier along that boundary. Consumed at reset
  to rewrite geom fields in place (no recompilation -- INV-1).
- build_model: compile the car XML once with a FIXED pool of 2*K parked box
  walls. Fixed topology for every episode/seed, exactly what makes the later
...
def wall_params(track, side, cfg: WallConfig):
    """Pose + half-length of K box walls along one boundary (side +1 left / -1 right).

    Consecutive boundary points joined by a thin vertical box; overlap keeps the
    wall continuous around curvature. Returns (pos (K,3), quat (K,4), half_len (K,)).
    """
    M = track.C.shape[0]
    ...
    # box local x = segment direction u, local z = up: yaw-only rotation about z.
    ang = np.arctan2(u[:, 1], u[:, 0])
    quat = np.stack(
        [np.cos(ang / 2), np.zeros(cfg.K), np.zeros(cfg.K), np.sin(ang / 2)], axis=1
    )
...
def build_model(cfg: WallConfig, car_xml_path=CAR_XML_PATH):
    """Compile the car XML once with a fixed pool of 2*K parked box walls.
    ...
    for i in range(2 * cfg.K):
        ...
        # size[0] placeholder half-length (along tangent), rewritten at reset.
        g.size[:3] = [0.1, cfg.half_thick, cfg.half_height]
    ...
    # ("same rotation as world body") and mj_kinematics then IGNORES our per-reset
    # geom_quat writes -- walls render/collide axis-aligned (a comb across the
    # track). Same stale-compile-cache trap as the midphase BVH above. Clear the
    # flag so geom_xmat is recomputed from geom_quat every step.
    model.geom_sameframe[wall_gid_start + np.arange(2 * cfg.K)] = 0
